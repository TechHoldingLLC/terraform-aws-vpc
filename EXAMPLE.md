...
> **Networking note:** the VPC is provisioned as dual stack. AWS assigns a `/56` IPv6 CIDR to the VPC and each public subnet receives a `/64` carved from that range. The public route table carries both `0.0.0.0/0` and `::/0` default routes pointing to the Internet Gateway. Private subnets and the private route table remain IPv4-only.
